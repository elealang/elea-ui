...
module UI.HTML.Comp.ProgramSwitcher (
    html
  ) where
...
html :: Page -> Html
html page = do
  H.div ! classes ["comp-program-switcher"] $ do
    H.div ! classes [cls "button"]
          ! X.onClick "contentModalActive = !contentModalActive" $ do
      H.div ! classes [cls "button-label"] $ do
        let st = Elea.programAbstractionName
        H.toMarkup $ fromMaybe "" $ Elea.programName page.program page.program.present
      H.div ! classes [cls "button-icon"] $
        H.preEscapedText $ fromJust $ 
          iconSVGWithName "expand-arrow" page.assets.iconIndex
